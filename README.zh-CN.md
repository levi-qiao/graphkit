...
host 有两种节奏。**自适应**（Codex、Claude Code 自定步的 `/loop`）在上一轮返回后才再唤起，一轮永远不会被打断。**定间隔**（Grok、Cursor、cron）要你设一个延时——`/graphkit` 会按一轮大概多久来算出这个间隔、填进启动命令里（Cursor 单次上限约 20m，超了会被杀，所以别超）。执行方和监督方跑在各自错开的排期上，谁都不会打断对方正在进行的一轮。
